...
# Curated Swift identifier names for enum values that either can't be expressed
# as a valid Swift 6.1 identifier (raw identifiers, SE-0451, only landed in
# Swift 6.2) or would produce a poor mechanical name. New entries added here
# take precedence over the mechanical fallback in swift_identifier() below.
_VALUE_NAME_OVERRIDES = {
    "urn:ietf:params:oauth:grant-type:jwt-bearer": "jwtBearer",
}

# Characters valid in librdkafka enum values that aren't valid Swift 6.1
# identifier characters (SE-0451 raw identifiers only landed in Swift 6.2, so
# backtick escaping alone isn't enough on the minimum supported toolchain).
_SWIFT_IDENTIFIER_SUBS = str.maketrans({":": "_", "-": "_"})


def swift_identifier(value: str) -> str:
    """Return a valid Swift identifier for a librdkafka enum value.

    Curated names in `_VALUE_NAME_OVERRIDES` win; otherwise fall back to a
    mechanical substitution that keeps the identifier valid on Swift 6.1.
    """
    if value in _VALUE_NAME_OVERRIDES:
        return _VALUE_NAME_OVERRIDES[value]
    return value.translate(_SWIFT_IDENTIFIER_SUBS)
