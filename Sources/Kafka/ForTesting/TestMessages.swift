...
public func _sendAndAcknowledgeMessages(
    ...
    for message in messages {
        guard acknowledgedMessages.contains(where: { $0.topic == message.topic }),
              acknowledgedMessages.contains(where: { $0.key == message.key.map({ ByteBuffer(string: $0) }) }),
              acknowledgedMessages.contains(where: { $0.value == message.value.map({ ByteBuffer(string: $0) }) }) else {
