...
public extension TestRDKafkaClient {
    ...
    /// - Parameter partitions: Number of partitions (`nil` by default - default in Kafka)
    ...
    func _createUniqueTopic(partitions: Int32? = nil, timeout: Int32) throws -> String {
        let uniqueTopicName = UUID().uuidString

        let errorChars = UnsafeMutablePointer<CChar>.allocate(capacity: TestRDKafkaClient.stringSize)
        defer { errorChars.deallocate() }

        guard let newTopic = rd_kafka_NewTopic_new(
            uniqueTopicName,
            partitions ?? -1, // use default num_partitions
            -1, // use default replication_factor
            errorChars,
            TestRDKafkaClient.stringSize
        ) else {
